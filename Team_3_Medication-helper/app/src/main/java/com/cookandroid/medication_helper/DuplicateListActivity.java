...
public class DuplicateListActivity extends AppCompatActivity {
    ...
    public void onBackPressed() {
        super.onBackPressed();
        Intent Back = new Intent(DuplicateListActivity.this, MedicineListActivity.class); // 복용 약 목록으로 돌아가는 기능
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        for(int i=0;i<listSize;i++){
                            ...
                            medicNameINGList[index][1]=dataSplit[0]; // 효능
                            medicNameINGList[index][2]=dataSplit[1]; // 금기명
                            medicNameINGList[index][3]=dataSplit[2]; // 성분
                            ...
                            sideRef.child(medicNameINGList[index][0]).addListenerForSingleValueEvent(new ValueEventListener() { // DB의 약품이름 항목에 대해
                                ...
                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                    if (!snapshot.child("mEffect").exists()) { // DB에 파싱한 약품의 효능중복 정보가 적혀있지 않고
                                        if (medicNameINGList[index][2] != null) { // 파싱한 약품이 효능중복 사항이 있다면
                                            updateSideCount("효능중복", 1); // 효능중복 사항이 있는 데이터가 하나 늘었음을 기록
                                            Map<String, Object> comForbidUpdate = new HashMap<>(); // DB 저장용 Map을 생성한 후
                                            if (medicNameINGList[index][3] != null) // 파싱한 데이터에 성분이 있다면
                                                comForbidUpdate.put("component", medicNameINGList[index][3]); // 성분을 Map에 저장
                                            if (medicNameINGList[index][1] != null) // 파싱한 데이터에 효능중복 사항이 있다면
                                                comForbidUpdate.put("mEffect", medicNameINGList[index][1]); // 해당 사항을 Map에 저장
