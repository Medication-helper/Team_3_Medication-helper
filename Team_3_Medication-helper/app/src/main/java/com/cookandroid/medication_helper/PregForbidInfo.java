/****************************
 ...
 설명 : 환자 - 선택한 약품에 대해 임부금기 정보를 제공합니다.
 ***************************/
...
public class PregForbidInfo extends AppCompatActivity {
    /* 의약품DB를 사용하기 위한 변수들 */
    UserData userData;
    String data;
    int listSize;
    ...
    public void onBackPressed() {
        super.onBackPressed();
        Intent Back = new Intent(PregForbidInfo.this, MedicineListActivity.class); // 복용 약 목록으로 돌아가는 기능
    ...
    protected void onCreate(Bundle savedInstanceState) {
            ...
            public void run() {
                ...
                ref.child(medicname).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        if (snapshot.hasChild("pForbid")) {
                            name.setText(medicname);
                            final String mEffect = snapshot.child("pForbid").getValue(String.class);
                                            ...
                                            ref.addListenerForSingleValueEvent(new ValueEventListener() {
                                                ...
                                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                    if (!snapshot.child("pForbid").exists()) { // DB에 파싱한 약품의 임부금기 정보가 적혀있지 않고
                                                        if (medicNameINGList[1] != null) { // 파싱한 약품이 임부금기 사항이 있다면
                                                            ...
                                                            if (medicNameINGList[3] != null) // 파싱한 데이터에 임부금기 사항이 있다면
                                                                comForbidUpdate.put("pForbid", medicNameINGList[3]); // 해당 사항을 Map에 저장
        ...
        btnOK.setOnClickListener(new View.OnClickListener() { // 뒤로가기 버튼을 눌렀을 경우
            ...
            public void onClick(View view) {
                Intent intent = new Intent(PregForbidInfo.this, MedicineListActivity.class); // 이전 화면으로 돌아가는 동작
